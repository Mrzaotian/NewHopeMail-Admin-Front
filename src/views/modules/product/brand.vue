<template>
  <div class="mod-config">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <img style="width:150px; height: 100px" :src="scope.row.logo" />
        </template>
      </el-table-column>
      <el-table-column
        prop="descript"
      ...
      >
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            @click="RelationDialog(scope.row.brandId)"
            >关联分类</el-button
          >
          <el-button
            type="text"
    ...
    ></add-or-update>
    <el-dialog title="关联分类" :visible.sync="relationVisible" width="30%">
      <el-popover placement="right-end" v-model="popVisible">
        <category-cascader :catelogPath.sync="catelogPath"></category-cascader>
        <div style="text-align: right; margin: 0">
          <el-button size="mini" type="text" @click="popVisible = false"
            >取消</el-button
          >
          <el-button type="primary" size="mini" @click="addRelation()"
            >确定</el-button
          >
        </div>
        ...
        <el-table-column fixed="right" header-align="center" align="center" label="操作">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="deleteRelation(scope.row.id,scope.row.brandId)"
            >移除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  components: { AddOrUpdate,CategoryCascader },
  data() {
    return {
      dataForm: {
        key: "",
  ...
  methods: {
    ...
    updateStatus(data) {
      ...
      this.$http({
        url: this.$http.adornUrl("/product/brand/update/switch"),
        method: "post",
        data: this.$http.adornData({ brandId, showStatus }, false),
      }).then(({ data }) => {});
    },
    RelationDialog(brandId) {
